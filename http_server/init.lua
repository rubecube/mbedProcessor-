...
server.post('/gpio_write', function(request, response)
  ...
  --print('Getting write')
  --print('the pin type: ' ..type(request.params.pin) ..'GPIO.state: '..type(request.params.state))
  --gpio.write(request.params.pin, request.params.state)
    print('in loop')
    gpio.write(request.params.pin,"1")
    tmr.delay(1000000)
    gpio.write(request.params.pin,"0")
  response.begin(function()
    response.finish()
  end)
end)
...
server.post('/pwm', function(request, response)
  pwm.setup(request.params.pin, 1, 0)
